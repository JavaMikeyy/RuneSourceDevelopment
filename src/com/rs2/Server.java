...
import com.rs2.model.players.Equipment;
...
public class Server implements Runnable {
	...
	private Map<SelectionKey, Player> playerMap;
	...
	public void run() {
		try {
			...
			ItemManager.getInstance().startup();
	...
	private void startup() throws IOException {
		...
		playerMap = new HashMap<SelectionKey, Player>();
	}
			...
			for (SelectionKey selectionKey : selector.selectedKeys()) {
				if (selectionKey.isValid()) {
					if (selectionKey.isAcceptable()) {
						...
						PacketManager.handleIncomingData(playerMap.get(selectionKey));
					}
				}
			}
		} catch (Exception ex) {
	 ...
	 */
	public Map<SelectionKey, Player> getPlayerMap() {
		return playerMap;
	}
