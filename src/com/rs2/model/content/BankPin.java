...
import com.rs2.model.content.dialogue.impl.Banker;
...
public class BankPin {
			...
			case CHANGING:
				for (int i = 0; i < 10; i ++)
					if (buttonId == 58025 + i) {
						...
						System.out.println("it's at " + interfaceStatus + " and i is " + i +
								" and pendingpin is at " + pendingBankPin[interfaceStatus]);
						if (interfaceStatus + 1 < 4) {
							sendBankPinVerificationInterface(interfaceStatus + 1);
						}
						else {
							Banker.sendDialogue(player, 13);
						}
					}	
				return;
		}
	...
	private void changeBankPin() {
		...
		pinAppendYear = -1;
		pinAppendDay = -1;
		player.getActionSender().sendMessage("Your bank pin pin has been successfully changed!");
	}
	...
	private void deleteBankPin() {
		...
		pinAppendYear = -1;
		pinAppendDay = -1;
		player.getActionSender().sendMessage("Your bank pin pin has been successfully deleted!");
	}
	...
	private void verifyPin(int pinEntered) {
		if (enteredCorrectedPin(pinEntered)) {
			if (interfaceStatus + 1 < 4)
				sendBankPinVerificationInterface(interfaceStatus + 1);
			else {
	...
	private boolean enteredCorrectedPin(int pinEntered) {
		return (bankPin[interfaceStatus] == pinEntered);
	}
