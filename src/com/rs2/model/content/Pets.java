...
public class Pets {
	...
	public static final int[][] PET_IDS =
	{//itemId, petId
	{1561, 768}
	};
	...
	public void registerPet(int itemId, int petId) {
		...
		this.itemId = itemId;
		NpcDefinition def = World.getDefinitions()[petId];
		this.pet = new Npc(def, petId);
		pet.setPosition(new Position(player.getPosition().getX() - 1, player.getPosition().getY()));
		pet.setSpawnPosition(new Position(player.getPosition().getX() - 1, player.getPosition().getY()));
