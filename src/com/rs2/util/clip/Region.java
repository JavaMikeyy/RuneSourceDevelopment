...
public class Region {
	...
	public static boolean tileClipped(Position position, int x, int y, int z, boolean isRunning) {
		//Yet to add running.
		...
		for (int i = 0; i < coordinateModifiers.length; i++) {
			if (x == coordinateModifiers[i][0] && y == coordinateModifiers[i][1]) {
				if (i == 0) {
					return blockedNorthWest(position.getX(), position.getY(), 0, false);
				}
				else if (i == 1) {
					return blockedNorth(position.getX(), position.getY(), 0, false);
				}
				else if (i == 2) {
					return blockedNorthEast(position.getX(), position.getY(), 0, false);
				}
				else if (i == 3) {
					return blockedWest(position.getX(), position.getY(), 0, false);
				}
				else if (i == 4) {
					return blockedEast(position.getX(), position.getY(), 0, false);
				}
				else if (i == 5) {
					return blockedSouthWest(position.getX(), position.getY(), 0, false);
				}
				else if (i == 6) {
					return blockedSouth(position.getX(), position.getY(), 0, false);
				}
				else if (i == 7) {
					return blockedSouthEast(position.getX(), position.getY(), 0, false);
				}
			}
		}
		return false;
	...
	public static boolean blockedNorth(int x, int y, int z, boolean isRunning) {
    	int modifier = (isRunning ? 2 : 1);
		return (getClipping(x, y + (1 * modifier), z) & 0x1280120) != 0;
    }
    public static boolean blockedEast(int x, int y, int z, boolean isRunning) {
    	int modifier = (isRunning ? 2 : 1);
		return (getClipping(x + (1 * modifier), y, z) & 0x1280180) != 0;
    }
    public static boolean blockedSouth(int x, int y, int z, boolean isRunning) {
    	int modifier = (isRunning ? 2 : 1);
		return (getClipping(x, y - (1 * modifier), z) & 0x1280102) != 0;
    }
    public static boolean blockedWest(int x, int y, int z, boolean isRunning) {
    	int modifier = (isRunning ? 2 : 1);
		return (getClipping(x - (1 * modifier), y, z) & 0x1280108) != 0;
    }
    public static boolean blockedNorthEast(int x, int y, int z, boolean isRunning) {
    	int modifier = (isRunning ? 2 : 1);
		return (getClipping(x + (1 * modifier), y + (1 * modifier), z) & 0x12801e0) != 0;
    }
    public static boolean blockedNorthWest(int x, int y, int z, boolean isRunning) {
    	int modifier = (isRunning ? 2 : 1);
		return (getClipping(x - (1 * modifier), y + (1 * modifier), z) & 0x1280138) != 0;
    }
    public static boolean blockedSouthEast(int x, int y, int z, boolean isRunning) {
    	int modifier = (isRunning ? 2 : 1);
		return (getClipping(x + (1 * modifier), y - (1 * modifier), z) & 0x1280183) != 0;
    }
    public static boolean blockedSouthWest(int x, int y, int z, boolean isRunning) {
    	int modifier = (isRunning ? 2 : 1);
		return (getClipping(x - (1 * modifier), y - (1 * modifier), z) & 0x128010e) != 0;
    }
    ...
    public static void load() {
    	try {
		System.out.println("Loading cache regions...");
		...
		System.out.println("Loaded cache regions.");
    	} catch (Exception e) {
    		e.printStackTrace();
    	}
    }
